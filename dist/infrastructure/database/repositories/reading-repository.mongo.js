...
const date_utils_1 = require("../../../domain/utils/date-utils");
...
class ReadingRepositoryMongo {
    // Se agrupa la lectura en un bucket por hora.
    async saveBucket(reading) {
        const dateBucket = new Date(reading.date.getFullYear(), reading.date.getMonth(), reading.date.getDate(), reading.date.getHours());
        await reading_model_1.default.findOneAndUpdate({
    ...
    async getTodayReadings(depositId, sensorId) {
        const sensorObjectId = new mongoose_1.Types.ObjectId(sensorId);
        const depositObjectId = new mongoose_1.Types.ObjectId(depositId);
        ...
        const buckets = await reading_model_1.default.find({
    ...
    async getWeeklyReadings(depositId, sensorId) {
        const sensorObjectId = new mongoose_1.Types.ObjectId(sensorId);
        const depositObjectId = new mongoose_1.Types.ObjectId(depositId);
    ...
    async getMonthlyReadings(depositId, sensorId) {
        const sensorObjectId = new mongoose_1.Types.ObjectId(sensorId);
        const depositObjectId = new mongoose_1.Types.ObjectId(depositId);
        const today = new Date();
        ...
        const dailyData = Array.from({ length: totalDaysInMonth }, () => ({ sum: 0, count: 0 }));
    ...
    async exportReadings(depositId, sensorIds, filter) {
        const sensorObjectIds = sensorIds.map(id => new mongoose_1.Types.ObjectId(id));
        const depositObjectId = new mongoose_1.Types.ObjectId(depositId);
        const { startDate, endDate } = (0, date_utils_1.getDateRangeFromFilter)(filter);
        const results = await reading_model_1.default.aggregate([
            {
                $match: {
                    deposit_id: depositObjectId,
    ...
    async getComplianceStats(depositId, filter) {
        const depositObjectId = new mongoose_1.Types.ObjectId(depositId);
        const deposit = await deposit_model_1.default.findById(depositObjectId).lean();
        if (!deposit || !deposit.sensors)
            return [];
        const { startDate, endDate } = (0, date_utils_1.getDateRangeFromFilter)(filter);
        const statsList = [];
        for (const sensor of deposit.sensors) {
            if (!sensor._id || !sensor.state)
                continue;
            const sensorObjectId = new mongoose_1.Types.ObjectId(sensor._id);
            ...
            for (const bucket of buckets) {
                if (bucket.readings) {
                    for (const r of bucket.readings) {
                        totalReadings++;
                        const evaluation = (0, threshold_utils_1.evaluateThreshold)(r.value, minVal, maxVal);
    ...
    async getReportAlerts(depositId, filter) {
        const depositObjectId = new mongoose_1.Types.ObjectId(depositId);
        const deposit = await deposit_model_1.default.findById(depositObjectId).lean();
        const countByType = {
            "Nivel": 0,
        ...
        for (const sensor of deposit.sensors) {
            ...
            for (const bucket of buckets) {
                if (bucket.readings) {
                    for (const r of bucket.readings) {
                        const evaluation = (0, threshold_utils_1.evaluateThreshold)(r.value, minVal, maxVal, deposit.name, sensor.unit || "");
                        if (evaluation.isTriggered) {
...
exports.default = ReadingRepositoryMongo;
